    ...
    <div class="detail-info">
      <div class="bt-right">
        ...
        <el-date-picker
                v-model="startTime"
                type="date"
                placeholder="选择审核通过开始日期" style="margin-left: 200px">
        </el-date-picker>
        --
        <el-date-picker
                v-model="endTime"
                type="date"
                placeholder="选择审核通过结束日期">
        </el-date-picker>
...
<script>
  import {getDormitoryMaintenanceInfo} from "../../network/housemaster";
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        getDormitoryMaintenanceInfo(this.pagination.currentPage, this.startTime, this.endTime)
            .then(res => {
              if (res.data.status) {
